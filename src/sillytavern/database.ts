...
export async function initializeDatabase(): Promise<void> {
  ...
  // Load user data if database is empty (first time)
  if (presetCount === 0 && lorebookCount === 0 && settingsCount === 0) {
    const userData = await loadUserData();

    if (userData) {
      await db.presets.add(userData.preset);
      await db.lorebooks.add(userData.lorebook);

      const settings: AppSettings = {
        ...DEFAULT_SETTINGS,
        key: 'settings',
        activePresetId: userData.preset.id,
        activeLorebookIds: [userData.lorebook.id],
        characterName: userData.characterName,
        userName: '玩家',
        formatPromptTemplate: DEFAULT_SETTINGS.formatPromptTemplate,
      };
      await db.settings.put(settings);

      // Create initial chat session with first message
      const firstMsg: ChatSession['messages'][number] = {
        id: crypto.randomUUID(),
        role: 'assistant',
        content: userData.firstMessage,
        timestamp: Date.now(),
        variablesAfter: userData.initialVariables,
      };

      const chat: ChatSession = {
        id: crypto.randomUUID(),
        name: `${userData.characterName} - 新游戏`,
        messages: [firstMsg],
        characterName: userData.characterName,
        userName: '玩家',
        presetId: userData.preset.id,
        lorebookIds: [userData.lorebook.id],
        variables: userData.initialVariables,
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
  ...
  // Fallback to default preset if no presets
  if (presetCount === 0) {
