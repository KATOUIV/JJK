...
export interface Lorebook {
  ...
  createdAt: number;
  updatedAt: number;
  /** Which API(s) should receive this lorebook content: primary=story API, secondary=variable API, both=all APIs */
...
export interface AppSettings {
  ...
  /** User's world preset selection */
  worldPreset?: string;
  /** Supplementary world settings text */
  worldExtra?: string;
  /** Content censorship toggle */
  censorship?: boolean;
  /** Strict compliance mode toggle */
  strictMode?: boolean;
}
...
变量更新（以下两种格式任选其一）：
1) <vars>{ "user.等级": 2, "user.EXP": 50, "user.持有金钱": 10500 }</vars>
2) <UpdateVariable>
<JSONPatch>
[
  { "op": "replace", "path": "/user/等级", "value": 2 },
  { "op": "delta", "path": "/user/EXP", "value": 50 }
]
</JSONPatch>
</UpdateVariable>

关键变量提醒：
- user.等级 / user.EXP / user.战力评级：战斗/训练结束后更新，EXP满100升1级，战力随等级自动判定
- user.名望.正道 / user.名望.邪道：数值-100~100，按任务阵营严格对应提升
- user.咒力.当前值 / user.咒力.最大值：使用术式消耗当前值，最大值消耗KP提升
- user.生得术式.熟练度 / user.战技.熟练度 / user.扩展术式.熟练度：每次有效使用+1，阶段自动判定（入门/熟练/精通）
- user.行囊 / user.束缚 / user.特殊体质 / user.咒灵操术：获得/失去时更新
- 任务系统：接取时完整填写，结算后删除
...
export interface VarsPatch {
  ...
  /** Optional JSON Patch array (RFC 6902 style) for variable updates */
  patch?: any[];
}
