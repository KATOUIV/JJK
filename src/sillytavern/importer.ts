...
export function importLorebook(data: SillyTavernLorebookExport): Omit<Lorebook, 'id' | 'createdAt' | 'updatedAt'> {
  ...
  const entries: LorebookEntry[] = rawEntries
    .map((e) => ({
      ...
      disabled: e.disable || e.excluded || false,
...
export function importPreset(data: Record<string, any>): Omit<ChatPreset, 'id' | 'createdAt' | 'updatedAt'> {
  ...
  // Normalize prompt_order: SillyTavern stores it as [{ character_id, order: [...] }]
  // We flatten it to [{ identifier, enabled, ... }]
  let promptOrder = data.prompt_order;
  if (Array.isArray(promptOrder) && promptOrder.length > 0) {
    const first = promptOrder[0];
    if (first && Array.isArray(first.order)) {
      promptOrder = first.order;
    }
  }
