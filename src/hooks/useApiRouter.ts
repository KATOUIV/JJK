...
export function useApiRouter(api: ApiSettings) {
  ...
  const sendRaw = useCallback(async (args: { task: Task; messages: Array<{ role: string; content: string }>; }): Promise<string> => {
    abortRef.current?.abort();
    abortRef.current = new AbortController();
    const { task, messages } = args;
    const { response } = await router.call(task, { messages, stream: false });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  return { sendStream, sendRaw, abort, targetFor: router.targetFor };
}
