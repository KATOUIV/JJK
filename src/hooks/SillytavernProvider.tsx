...
export function SillytavernProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const primaryPresets = p.filter((preset) => preset.apiTarget === 'primary' || preset.apiTarget === 'both' || !p.apiTarget);
  ...
  return (
    <SillytavernContext.Provider value={value}>
      {children}
    </SillytavernContext.Provider>
  );
}
