...
function CurseCard({
  ...
  return (
    <div style={{ position: 'relative', ...style }}>
      <fieldset style={{ border: `1px solid ${borderColor}`, borderRadius: 4, padding: 12, margin: 0 }}>
        {children}
      </fieldset>
      <div style={{ ...cornerStyle, top: -1, left: -1, borderTop: `2px solid ${borderColor}`, borderLeft: `2px solid ${borderColor}` }} />
      <div style={{ ...cornerStyle, top: -1, right: -1, borderTop: `2px solid ${borderColor}`, borderRight: `2px solid ${borderColor}` }} />
      <div style={{ ...cornerStyle, bottom: -1, left: -1, borderBottom: `2px solid ${borderColor}`, borderLeft: `2px solid ${borderColor}` }} />
      <div style={{ ...cornerStyle, bottom: -1, right: -1, borderBottom: `2px solid ${borderColor}`, borderRight: `2px solid ${borderColor}` }} />
    </div>
  );
}
    ...
    >
      <div
        ...
        style={{
          ...
          padding: 16,
          color: 'var(--jjk-text)',
          borderLeft: '1px solid rgba(170,0,0,0.35)',
        }}
      >
        <header style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, alignItems: 'center' }}>
          <strong style={{ color: 'var(--jjk-text)' }}>设置</strong>
          <button onClick={onClose} style={{ ...btnBase, padding: '2px 8px' }}>×</button>
        </header>

        <div style={{ display: 'flex', gap: 4, marginBottom: 16, borderBottom: '1px solid rgba(170,0,0,0.25)', paddingBottom: 8, flexWrap: 'wrap' }}>
          {TABS.map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              style={{
                padding: '4px 10px',
                border: '1px solid rgba(170,0,0,0.3)',
                background: tab === t ? 'rgba(170,0,0,0.25)' : 'rgba(28,28,28,0.6)',
        ...
        {tab === 'primary' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <label>
              <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>API 模式</span>
              <select
              ...
              <small style={{ display: 'block', color: 'var(--jjk-text-4)', marginTop: 4, fontSize: 11 }}>
                {isDual
            ...
            <label>
              <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>Base URL</span>
              <input
                type="text"
            ...
            <label>
              <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>API Key</span>
              <input
                type="password"
            ...
            <label>
              <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>Model</span>
              <input
                type="text"
            ...
            <hr style={{ border: 'none', borderTop: '1px solid rgba(170,0,0,0.2)', margin: '8px 0' }} />
            <label>
              <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>用户名</span>
              <input
                type="text"
            ...
            <label>
              <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>角色名</span>
              <input
                type="text"
        ...
        {tab === 'secondary' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {!isDual && (
              <div style={{ padding: 10, background: 'rgba(170,140,0,0.12)', border: '1px solid rgba(170,140,0,0.35)', borderRadius: 4, fontSize: 12, color: 'var(--jjk-gold)' }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <label style={{ flex: 1 }}>
                <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>温度 (0-2)</span>
                <input
                  type="number"
                  min={0}
                  max={2}
              ...
              <label style={{ flex: 1 }}>
                <span style={{ color: 'var(--jjk-text-2)', fontSize: 13 }}>Max Tokens</span>
                <input
                  type="number"
                  min={1}
                  max={32768}
        ...
        {tab === 'tags' && (
          <div>
            <p style={{ fontSize: 12, color: 'var(--jjk-text-4)', marginBottom: 12 }}>
              ...
              {settings.customTags.map((t, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--jjk-text-4)' }}
                  >
                    ×
                  </button>
                </span>
        ...
        {tab === 'display' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <CurseCard>
              <legend style={{ fontSize: 14, fontWeight: 'bold', color: 'var(--jjk-text-2)' }}>思考过程显示</legend>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
                {(['fold', 'hide', 'inline'] as const).map((m) => (
                  <label key={m} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', color: 'var(--jjk-text-3)' }}>
                    <input
                      type="radio"
            ...
            <CurseCard>
              <legend style={{ fontSize: 14, fontWeight: 'bold', color: 'var(--jjk-text-2)' }}>UI 模式</legend>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
        ...
        {tab === 'backup' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <CurseCard borderColor="rgba(0,170,80,0.4)">
              <legend style={{ fontSize: 14, fontWeight: 'bold', color: 'rgba(0,200,100,0.9)' }}>导出</legend>
              <p style={{ fontSize: 12, color: 'var(--jjk-text-4)', marginBottom: 8 }}>
                将所有世界书、预设、设置、对话导出为单个 JSON 文件。
              </p>
              <button
                onClick={handleExportAll}
                style={{ padding: '8px 16px', background: 'rgba(0,150,70,0.8)', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}
              >
                导出全部数据
              </button>
            </CurseCard>
            <CurseCard borderColor="rgba(100,100,170,0.4)">
              <legend style={{ fontSize: 14, fontWeight: 'bold', color: 'rgba(130,130,220,0.9)' }}>导入</legend>
              <p style={{ fontSize: 12, color: 'var(--jjk-text-4)', marginBottom: 8 }}>
                从之前导出的备份文件恢复数据。<strong style={{ color: 'var(--jjk-red-hi)' }}>会覆盖现有数据</strong>。
              </p>
              <button
                onClick={handleImportAll}
                style={{ padding: '8px 16px', background: 'rgba(100,100,180,0.8)', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}
              >
                导入备份文件
              </button>
            </CurseCard>
            <CurseCard borderColor="rgba(170,0,0,0.4)">
              <legend style={{ fontSize: 14, fontWeight: 'bold', color: 'var(--jjk-red-hi)' }}>清除</legend>
              <p style={{ fontSize: 12, color: 'var(--jjk-text-4)', marginBottom: 8 }}>
                清除所有本地存储数据。<strong style={{ color: 'var(--jjk-red-hi)' }}>不可恢复</strong>。
              </p>
              <button
                onClick={handleClearAll}
                style={{ padding: '8px 16px', background: 'rgba(170,0,0,0.7)', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}
              >
                清除所有数据
              </button>
            </CurseCard>
